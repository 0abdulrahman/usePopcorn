...
function StarRating({ length, setLength }) {
      ...
      <div className="stars">
        {Array.from({ length: 10 }, (_, i) => (
          <Star
            ...
            key={i}
            full={tempLength > 0 ? i + 1 <= tempLength : i + 1 <= length}
            onMouseEnter={() => setTempLength(i + 1)}
            onMouseLeave={() => setTempLength(0)}
          />
        ))}
      </div>
      <p>{tempLength || length || ""}</p>
    </div>
  );
}
